import type { GenesisCheckpoint, GenesisProgress } from "./types";

export const GENESIS_CHECKPOINTS: GenesisCheckpoint[] = [
  {
    index: 0,
...
export function initialGenesisProgress(): GenesisProgress {
  return { checkpointIndex: 0, momentum: 0, history: [] };
}

export function checkpointFor(progress: GenesisProgress): GenesisCheckpoint | null {
  return GENESIS_CHECKPOINTS[progress.checkpointIndex] ?? null;
}

export function resolveChoice(seed: string, progress: GenesisProgress, choiceId: string): GenesisProgress {
