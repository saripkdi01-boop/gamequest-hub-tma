...
export function GameCanvas({ checkpointIndex, focusedGate, onGateFocus }: GameCanvasProps) {
  ...
  return <div className="relative h-[39svh] min-h-[260px] max-h-[390px] overflow-hidden rounded-2xl border border-white/10 bg-[#081326] shadow-[0_18px_48px_rgba(0,0,0,.36)]"><canvas ref={canvasRef} className="block h-full w-full touch-pan-y" aria-label={t("routeAtlas")} />{!ready && <div className="absolute inset-0 grid place-items-center bg-[#081326] font-mono text-[9px] uppercase tracking-[.14em] text-[#d7fb70]">{t("syncingRoute")}</div>}</div>;
}
