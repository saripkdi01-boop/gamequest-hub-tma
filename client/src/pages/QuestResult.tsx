...
export default function QuestResult() {
  ...
  if (!result) return <div className="game-shell grid min-h-[100dvh] place-items-center p-4 text-[#fbf8ed]"><main className="quest-nexus-panel w-full max-w-[420px] p-5 text-center"><div className="relative z-[1]"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-white/[.06] text-[#aebac4]"><Sparkles size={22} /></div><h1 className="mt-4 font-display text-[26px] leading-none">{t("resultUnavailable")}</h1><button onClick={() => setLocation("/")} className="btn-3d btn-quest mt-5 w-full py-3 text-xs">{t("returnHub")}</button></div></main></div>;
