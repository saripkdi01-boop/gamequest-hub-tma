    ...
    <div className="game-shell min-h-[100dvh] overflow-x-hidden pb-[calc(var(--tg-safe-area-inset-bottom)+28px)]">
      <main className="mx-auto w-full max-w-[520px] px-5 pt-[calc(var(--tg-content-safe-area-inset-top)+18px)]">
        <header className="flex items-center justify-between">
          ...
          <button onClick={() => setLocation("/leaderboard")} className="flex h-9 items-center gap-2 rounded-full border border-white/10 bg-white/[.045] px-3 text-[10px] font-medium text-[#d7fb70]">
            <span className="h-1.5 w-1.5 rounded-full bg-[#d7fb70] shadow-[0_0_10px_#d7fb70]" />
            {loading ? "Syncing" : hasVerifiedTelegramContext ? "Online" : "Preview"}
          </button>
        </header>
        ...
        <section className="relative mt-9 overflow-hidden rounded-[28px] border border-white/[.12] bg-[#1a2639]/80 px-6 pb-6 pt-7 shadow-[0_24px_60px_rgba(0,0,0,.26)]">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-[.16em] text-[#d7fb70]"><Sparkles size={13} /> Player card</div>
            <div className="mt-7">
              <p className="font-mono text-[10px] uppercase tracking-[.15em] text-[#9fae9d]">{hasVerifiedTelegramContext ? "Selamat datang kembali" : "Browser preview"}</p>
