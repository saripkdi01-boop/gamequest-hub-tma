...
function asProgress(value) {
  const parsed = z.object({ checkpointIndex: z.number().int().min(0).max(3), momentum: z.number().int(), history: z.array(z.object({ checkpoint: z.number().int(), choiceId: z.string(), momentum: z.number().int() })) }).safeParse(value);
  return parsed.success ? parsed.data : initialGenesisProgress();
}
async function getGameDashboard(player) {
  const supabase = getSupabaseServerClient();
  const [{ data: quest, error: questError }, { data: activeRun, error: runError }, { data: daily, error: dailyError }] = await Promise.all([supabase.from("quests").select("id,title,description,reward_xp,reward_relics").eq("slug", "genesis-run").eq("active", true).single(), supabase.from("player_quests").select("id,status,progress_json,quest_id").eq("player_id", player.id).eq("status", "active").order("started_at", { ascending: false }).limit(1).maybeSingle(), supabase.from("daily_player_stats").select("completed_quests,rewarded_ads_count").eq("player_id", player.id).eq("day_utc", todayUtc()).maybeSingle()]);
  ...
  return { player: { id: player.id, firstName: player.firstName, username: player.username, level: player.level, experience: player.experience, experienceToNextLevel: experienceToNextLevel(player.experience), questStreak: player.questStreak, relics: player.relics }, genesisRun: { id: activeRun?.id ?? null, status: activeRun?.status ?? "available", title: quest.title, description: quest.description, rewardXp: quest.reward_xp, rewardRelics: quest.reward_relics, checkpointIndex: progress.checkpointIndex }, daily: { completedQuests: daily?.completed_quests ?? 0, rewardedAdsCount: daily?.rewarded_ads_count ?? 0 } };
}
...
async function createGenesisRun(playerId, questId) {
  const { data, error } = await getSupabaseServerClient().from("player_quests").insert({ player_id: playerId, quest_id: questId, status: "active", seed: randomUUID(), progress_json: initialGenesisProgress(), started_at: (/* @__PURE__ */ new Date()).toISOString() }).select("id,status,seed,progress_json,quest_id").single();
  if (error || !data) {
...
async function startExistingRun(playerId, questId) {
  const { data, error } = await getSupabaseServerClient().from("player_quests").select("id,status,seed,progress_json,quest_id").eq("player_id", playerId).eq("quest_id", questId).eq("status", "active").single();
...
async function submitGenesisChoice(player, rawInput) {
  ...
  if (run.status === "completed") return { runId: run.id, status: "completed", checkpoint: null, momentum: currentProgress.momentum, result: { success: true, experience: player.experience, level: player.level, relics: player.relics, xpAwarded: 0, relicsAwarded: 0 } };
  ...
  return { runId: run.id, status: "completed", checkpoint: null, momentum: nextProgress.momentum, result: { success: true, experience: completed.experience, level: completed.level, relics: completed.relics, xpAwarded: completed.xpAwarded, relicsAwarded: completed.relicsAwarded } };
}
async function getLeaderboard(limit = 20) {
  ...
  return (rows ?? []).map((row, index) => ({ rank: index + 1, score: row.score, player: playerById.get(row.player_id) ?? { first_name: "Pathfinder", username: null, level: 1 } }));
}
export {
  getGameDashboard,
  getLeaderboard,
