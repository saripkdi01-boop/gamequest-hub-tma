...
export type TelegramUpdate = { update_id: number; message?: { text?: string; chat: { id: number } } };
...
export function verifyTelegramInitData(initData: string, botToken = process.env.TELEGRAM_BOT_TOKEN, nowSeconds = Math.floor(Date.now() / 1000)): TelegramUser {
  ...
  if (authDate > nowSeconds + 60 || nowSeconds - authDate > 86_400) throw new TelegramValidationError("Expired initData");
  ...
  try {
    ...
    return user;
  } catch { throw new TelegramValidationError("Invalid user payload"); }
}
...
function commandFrom(text: string): "start" | "run" | null {
