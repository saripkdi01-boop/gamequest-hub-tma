...
export type AdQuestPlacement = "daily_bonus" | "revive_genesis_run" | "signal_mining" | "relic_resonance";
export type AdRewardCurrency = "xp" | "relic" | "quest_coin";
export type RewardAdProvider = "monetag" | "adsgram";

type AdQuestDefinition = { title: string; description: string; rewardCurrency: AdRewardCurrency; rewardAmount: number };
const AD_QUESTS: Record<AdQuestPlacement, AdQuestDefinition> = {
...
const intentSchema = z.object({ placement: z.enum(["daily_bonus", "revive_genesis_run", "signal_mining", "relic_resonance"]).default("daily_bonus") });
...
export async function createRewardedAdIntent(player: GameQuestPlayer, rawInput: unknown) {
...
export const createDailyBonusIntent = createRewardedAdIntent;
...
export async function processMonetagPostback(rawInput: unknown) {
  ...
  if (logError) { if (logError.code === "23505") return { accepted: true, duplicate: true, rewarded: false }; throw new Error(logError.message); }
  ...
  if (!eligible) { await supabase.from("ad_reward_intents").update({ status: "rejected" }).eq("ymid", input.ymid).eq("status", "pending"); return { accepted: true, duplicate: false, rewarded: false }; }
  ...
  if (playerError || !player || (input.telegram_id && Number(input.telegram_id) !== Number(player.telegram_user_id))) { await supabase.from("ad_reward_intents").update({ status: "rejected" }).eq("ymid", input.ymid).eq("status", "pending"); return { accepted: true, duplicate: false, rewarded: false }; }
  ...
  const result = rewardResult as { rewarded: boolean; duplicate: boolean; currency?: string; amount?: number };
