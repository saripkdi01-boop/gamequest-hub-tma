import type { GuideState } from "./guide-service";
...
export type GameDashboard = {
  player: {
    id: string; firstName: string; username: string | null; level: number; experience: number; experienceToNextLevel: number;
    questStreak: number; relics: number; questCoins: number; mindScore: number; dailyScore: number; energy: number;
    maxEnergy: number; comboBest: number; activeGuideId: string;
  };
  genesisRun: { id: string | null; status: QuestStatus | "available"; title: string; description: string; rewardXp: number; rewardRelics: number; checkpointIndex: number };
  daily: { completedQuests: number; rewardedAdsCount: number; correctAnswers: number; qcEmitted: number; dailyScore: number };
  guideState: GuideState;
  dailyLogin: { streakDay: number; claimedToday: boolean; nextRewardRelics: number; rewardTrack: number[] };
